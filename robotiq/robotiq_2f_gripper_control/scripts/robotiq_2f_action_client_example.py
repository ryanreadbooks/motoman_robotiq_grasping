...
def operate_gripper():
    ...
    rospy.loginfo("Client test: Starting sending goals")
    ...
    while not rospy.is_shutdown():
        ...
        rospy.sleep(1)
        rospy.loginfo('finished one')
        Robotiq.goto(robotiq_client, pos=0.04, speed=0.01, force=10)
        rospy.sleep(1)
        rospy.loginfo('finished two')
        Robotiq.goto(robotiq_client, pos=0.011, speed=0.01, force=0 , block=True)
        rospy.sleep(1)
        rospy.loginfo('finished three')
        Robotiq.goto(robotiq_client, pos=0.14, speed=0.06, force=100 , block=True)
        rospy.sleep(1)
        rospy.loginfo('finished four')
