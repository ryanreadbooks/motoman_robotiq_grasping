...
from .model.loss_utils import GRASP_MAX_WIDTH, batch_viewpoint_params_to_matrix, \
...
def process_grasp_labels(end_points):
    ...
    for i in range(len(clouds)):
        ...
        poses = end_points['object_poses_list'][i]  # [(3, 4),]   # 得到一个batch中所有物体的poses
        ...
        for obj_idx, pose in enumerate(poses):
            # 这里for循环是每个物体每个物体一一处理
            grasp_points = end_points['grasp_points_list'][i][obj_idx]  # (Np, 3)，在物体坐标系上的抓取点坐标
            grasp_labels = end_points['grasp_labels_list'][i][obj_idx]  # (Np, V, A, D)，每个抓取的得分score，由friction coefficient来
            grasp_offsets = end_points['grasp_offsets_list'][i][obj_idx]  # (Np, V, A, D, 3)，每个grasp的操作参数
            grasp_tolerance = end_points['grasp_tolerance_list'][i][obj_idx]  # (Np, V, A, D)
            _, V, A, D = grasp_labels.size()
            num_grasp_points = grasp_points.size(0)  # 抓取点的数量
            # generate and transform template grasp views
            grasp_views = generate_grasp_views(V).to(pose.device)  # (V, 3), V=300，这里是生成每个方向向量的值
            grasp_points_trans = transform_point_cloud(grasp_points, pose, '3x4')  # 将在物体坐标系上的抓取点用pose转换到相机坐标系下
            grasp_views_trans = transform_point_cloud(grasp_views, pose[:3, :3], '3x3')  # 将物体坐标系下的抓取方向向量也要转换到相机坐标系下
            ...
            grasp_views_rot = batch_viewpoint_params_to_matrix(-grasp_views, angles)  # (V, 3, 3) 将抓取方向向量和绕着这个向量的旋转角度转化成旋转矩阵
            grasp_views_rot_trans = torch.matmul(pose[:3, :3], grasp_views_rot)  # (V, 3, 3) ，将上一行算出来的旋转矩阵转到相机坐标系下

            # assign views
            # grasp_views是局部坐标系下的，在上半球上
            grasp_views_ = grasp_views.transpose(0, 1).contiguous().unsqueeze(0)
            # grasp_views_trans是已经转换到相机坐标系下了
            grasp_views_trans_ = grasp_views_trans.transpose(0, 1).contiguous().unsqueeze(0)
            # 用knn找grasp_views_和grasp_views_trans_哪个和哪个相近
            view_inds = knn(grasp_views_trans_, grasp_views_, k=1).squeeze() - 1
            # 从grasp_views_trans中选出相近的
        ...
        # 找到点云grasp_points_merged中和seed_xyz中最相近的点
        ...
        # 进一步挑选得到每个， Ns应该可以认为是最终用作训练的抓取点数量？
